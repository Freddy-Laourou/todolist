...
function format_date_affichage(date_iso) {
    const [annee, mois, jour] = date_iso.split('-');
    return `${jour}-${mois}`;
}
...
async function add_task() {
    ...
    if (texte_actuel != '' && date_value != '' && hour_value != '') {
        ...
        if (tache_en_edition != null) {
            ...
            date.textContent = format_date_affichage(date_value);
            date.dataset.full = date_value;
...
async function push_tasks() {
    ...
    let reponse = await fetch('/api/tasks');
...
function edit_task(tache) {
    ...
    date_saisie.value = date.dataset.full;
...
function trier_tasks() {
    ...
    non_selected_tasks.sort((a,b) => {
        const date_a = a.querySelector('.date').dataset.full;
        const heure_a = a.querySelector('.hour').textContent;
        const date_b = b.querySelector('.date').dataset.full;
